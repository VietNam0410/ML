...
def main():
    ...
    #         input_data[col] = st.selectbox(f"Chọn giá trị cho {col}", unique_values)
    #     else:
    #         input_data[col] = st.slider(f"{col}", float(df[col].min()), float(df[col].max()), float(df[col].median()))

    # input_df = pd.DataFrame([input_data])
    # original_input_df = input_df.copy()
    # input_df[numeric_cols] = scaler.transform(input_df[numeric_cols])  # Chuẩn hóa dữ liệu đầu vào

    # st.subheader("Dữ liệu gốc bạn đã nhập:")
    # st.dataframe(original_input_df)

    # if st.button("Dự đoán sống sót"):
    #     prediction = model.predict(input_df)
    #     survival_probability = prediction[0] * 100
    #     st.subheader(f"Xác suất sống sót dự đoán: {survival_probability:.2f}%")
    #     if survival_probability > 50:
    #         st.success("Hành khách này có khả năng sống sót cao!")
    #     else:
    #         st.error("Hành khách này có nguy cơ không sống sót!")
